...
luagram_helper = {
      ['add_events'] = ' function > add_events(def)',
      ...
      ['getActiveSessions'] = ' function > getActiveSessions(callback, data)',
...
function function_core.change_table(input, changes, rems)
  if type(input) == 'table' and type(changes) == 'table' and type(rems) == 'table' then
    local res = {}
    for key,value in pairs(input) do
      ...
      if type(value) ~= 'table' then
        ...
        res[key] = function_core.change_table(value, changes, rems)
      end
    end
    return res
  else
...
function function_core.run_table(table)
  local result = function_core.change_table(
  client:execute(
    function_core.change_table(
    table,
    {
...
function function_core.send_tdlib(table)
  local change ,rem = {
    luagram = string.reverse('epyt@')
  }, {}
  res = function_core.change_table(table,change,rem)
  client:send(res)
end
...
function luagram_function.add_events(def)
  if type(def) ~= 'function' then
    function_core.print_error('the add_events input must be a function !')
      return {
        luagram = false,
      }
    else
    update_functions[#update_functions + 1] = def
    return {
      luagram = true,
    }
  end
...
function luagram_function.help(def_name)
  ...
  elseif luagram.luagram_helper[def_name] then
    print(luagram_function.colors(' - %{yellow} '..luagram.luagram_helper[def_name]))
    return luagram.luagram_helper[def_name]
  else
...
function luagram_function.setChatMemberStatus(chat_id, user_id, status, right)
  ...
  chat_member_statusluagram = 'chatMemberStatus' .. status
  return function_core.run_table{
...
function luagram_function.getActiveSessions(callback, data)
  return function_core.run_table{
...
function luagram.run(main_def)
  if type(main_def) ~= 'function' then
    function_core.print_error('the run input must be a main function !')
    os.exit(1)
  else
    update_functions[0] = main_def
  end
  while true do
    ...
    local update = function_core.change_table(
    client:get(1),
    {
      _ = string.reverse('margaul'),
    },{
    ...
    if update then
      ...
      luagram.login(update.authorization_state or update)
      if update.message and type(update.message) == 'table' then
        for i = 0 , #update_functions do
          xpcall(update_functions[i], function_core.print_error,update.message)
        end
      end
    end
    ::finish::
...
function luagram.set_config(data)
  ...
  if not data.api_hash then
    io.write(luagram_function.colors('%{red} please use api_hash in your script !'))
    os.exit()
  end
  if not data.api_id then
    io.write(luagram_function.colors('%{red} please use api_id in your script !'))
    os.exit()
  end
  if not data.session_name then
    io.write(luagram_function.colors('%{red} please use session_name in your script !'))
    os.exit()
  end
  if not data.token and not luagram_function.exists('.luagram-'..data.session_name) then
    io.write(luagram_function.colors('%{green} please use your token or phone number > '))
  ...
  luagram.config.parameters = {
    ...
    device_model = 'luagram',
    system_version = '1.0',
    application_version = data.application_version or '1.0',
...
function luagram.login(state)
  ...
  elseif state.luagram == 'error' and state.message == 'PHONE_CODE_INVALID' then
    io.write(luagram_function.colors('%{green} code > '))
    local code = io.read()
    function_core.send_tdlib(
    {
  ...
  elseif state.luagram == 'error' and state.message == 'PASSWORD_HASH_INVALID' then
    ...
    io.write(luagram_function.colors('%{green} password > '))
  ...
  elseif state.luagram == 'authorizationStateWaitPassword' then
      io.write(luagram_function.colors('%{green} password [ '..state.password_hint..' ] > '))
      local password = io.read()
      function_core.send_tdlib(
      {
        luagram = 'checkAuthenticationPassword',
